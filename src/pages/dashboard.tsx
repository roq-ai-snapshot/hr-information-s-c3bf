...
import { getHrInformations } from 'apiSdk/hr-informations';
...
function DashboardPage() {
  ...
  const hrInformationStat = useSWR<PaginatedInterface<HrInformationInterface>>(
    () => `/hr-informations`,
    () => getHrInformations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Hr Informations',
      result: hrInformationStat?.data?.totalCount,
      isLoading: hrInformationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Payrolls',
